...
class tkinterApp(tk.Tk):
	...
	def __init__(self, *args, **kwargs):
		...
		for F in (Login, perfil_usuario, Perfil_usuario_receitas, Perfil_amigo, mostrar_receita_usuario, mostrar_receita_amigo, adicionar_receita, adicionar_amigo, calcular_calorias):
			if F == Login:
...
class perfil_usuario(tk.Frame):
	...
	def commando(self, nome_amigo):# mostra o perfil do amigo
		if nome_amigo != "":
			...
			for i in range(len(self.amigo_lista_receita)): # limpa a lista atual para receber a nova lista de receitas
				self.amigo_lista_receita[i].set("")
...
class calcular_calorias(tk.Frame):
    
	def __init__(self, parent, controller):
		self.controller = controller


		self.lista_ingredientes = [] # lista de ingredientes com a quantidade, nome e calorias
		...
		# Titulo da receita
		self.label = ttk.Label(self, text ="Calcular Calorias", font = MIDFONT)
		self.label.place(x = 10,y = 10)

  
		# Adicionar Titulo
		self.quantidadeLabel = Label(self, text="Quantidade (gramas)", font=("Arial", "14"))
		self.quantidadeLabel.place(x = 8,y = 60)
  
		self.conteiner_quantidade = Frame(self)
		self.conteiner_quantidade.place(x = 8,y = 88)

		self.titulo_receita=tk.StringVar()
		self.titulo = Entry(self.conteiner_quantidade, textvariable = self.titulo_receita)
		self.titulo["width"] = 40 # largura da caixa de texto
		self.titulo["font"] = self.fontePadrao
		self.titulo.pack(side=LEFT)
		...
		# Adicionar Ingredientes
		self.ingredientesLabel = Label(self, text="Nome ingrediente:", font=("Arial", "14"))
		self.ingredientesLabel.place(x = 8,y = 130)
		...
		# Boatão que adiciona os ingredianets
		self.add_ingrediente = ttk.Button(self.conteiner_ingredientes, text ="Add", command = lambda : self.listar_ingredientes())
		self.add_ingrediente.pack(side=LEFT, padx= 5)
		self.listar_ingredienetesLabel = Label(self, text="", font=("Arial", "9"), justify=LEFT)
		self.listar_ingredienetesLabel.place(x = 8, y = 200)
		...
		self.calorias_total_label = Label(self, text="", font=("Arial", "9"), justify=LEFT)
		self.calorias_total_label.place(x = 160, y = 610)

	def get_calorias(self, ingrediente, quantidade):
		url = "http://127.0.0.1:5000/hello/"+ ingrediente + "/" + str(quantidade)
		response = requests.get(url)
		if response.status_code == 200:
        	# Capturar o conteúdo da resposta
			mensagem = response.text
		else:
			# Se a requisição falhou, exibir mensagem de erro
			mensagem = "Erro"
	...
	def listar_ingredientes(self):
			...
			self.ingrediente = self.ingredientes.get()
			# coleta a quantidade de ingrediente
			self.quantidade = self.titulo_receita.get()

			self.titulo_receita.set("")  
			self.ingredientes.delete(0, tk.END)
   
			if self.ingrediente != "":
				#coleta o valor das calorias
				self.calorias = self.get_calorias(self.ingrediente, int(self.quantidade)) 
				...
				if self.calorias != "Erro":
					...
					self.calorias_total_label["text"] = "Total de calorias: " + str(self.calorias_total)
					inicio = len(self.lista_ingredientes)-20 if len(self.lista_ingredientes) >= 20 else 0 # exibi apenas os ultimos 15 ingredientes	

					for i in range(inicio, len(self.lista_ingredientes)):
						self.texto += self.lista_ingredientes[i] + "\n"
					self.listar_ingredienetesLabel["text"] = self.texto
				else:
					self.calorias_total_label["text"] = "Ingrediente desconhecido"
